   ...
   public class CrudAdminDominio
    {
        ...
        public DataTable MostrarDonaciones()
        {
            DataTable tabla = new DataTable();
            tabla = crudAdmin.MostrarDonaciones();
            return tabla;
        }

        public void IngresoDonacion(string nombre, string cantidad)
        {
            crudAdmin.IngresoDonacion(nombre, Convert.ToInt32(cantidad));
        }

        public void IngresoGasto(string nombre, string cantidad)
        {
            crudAdmin.IngresoGasto(nombre, Convert.ToInt32(cantidad));
        }

        public DataTable MostrarGastos()
        {
            DataTable tabla = new DataTable();
            tabla = crudAdmin.MostrarGastos();
            return tabla;
        }
    }
}
